...
const Pagination = ({ context }) => {
  const { total, currentPage, totalPage, pageSize, paginationPrefix = "" } = context || {}
  const seq = [1, 2, 3, 4, 5]
  const prevSeq = seq.map(n => currentPage - n).filter(n => n > 0).reverse()
    ...
    <Container show={currentPage !== undefined && totalPage > 1}>
      <LeftSide>
        <NavLink
          ...
          to={`${paginationPrefix}/article-list/1`}
        >
          <Icon type="s" id="angle-double-left" />
        </NavLink>
        <NavLink
          disable={currentPage <= 1}
          to={`${paginationPrefix}/article-list/${currentPage - 1}`}
        >
      ...
      <CenterSide>
        ...
        {prevSeq.map(i => (
          <NavLink
            key={i}
            disable={false}
            to={`${paginationPrefix}/article-list/${i}`}
          >
            {i}
          </NavLink>
        ))}
      ...
      <RightSide>
        ...
        <NavLink
          disable={currentPage >= totalPage}
          to={`${paginationPrefix}/article-list/${totalPage}`}
        >
